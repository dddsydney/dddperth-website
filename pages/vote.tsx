...
class VotePage extends React.Component<VoteProps, VoteState> {
  ...
  setSessions(sessions: DddSession[]) {
    ...
    if (!(this.isRunningInBrowser() && localStorage)) {
      ...
      if (orderings == null) {
        const ids = JSON.stringify(sessions.map(({ SessionId }) => SessionId)) // Randomizing will be done in backend API
            ...
            <div className="col-md-8" style={{ backgroundColor: '#f5f5f5', padding: '0 20px' }}>
              ...
              <ul>
                ...
                <li>
                  ...
                  Eventbrite ticket order # (from your confirmation email usually sent from: Ti.to &lt;orders@ti.to&gt;)
                  if you've{' '}
                  <Link href="/tickets">
